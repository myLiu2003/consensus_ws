...
Multi_UAV::Multi_UAV(rclcpp::Node* node, const int & drone_id_) {
    node_ = node;
    drone_id = drone_id_;
    ...
    QuadState_subscriber = node_->create_subscription<swarm_msgs::msg::QuadStatePub>(sub_quadstate_topic_name, 1000, [this](const swarm_msgs::msg::QuadStatePub::SharedPtr m){ QuadstateCbk(m); });
    QuadState_publisher = node_->create_publisher<swarm_msgs::msg::QuadStatePub>("/quadstate_to_teammate", 1000);
    GlobalExtrinsic_subscriber = node_->create_subscription<swarm_msgs::msg::GlobalExtrinsicStatus>(sub_global_extrinsic_topic_name, 1000, [this](const swarm_msgs::msg::GlobalExtrinsicStatus::SharedPtr m){ GlobalExtrinsicCbk(m); });
    GlobalExtrinsic_publisher = node_->create_publisher<swarm_msgs::msg::GlobalExtrinsicStatus>("/global_extrinsic_to_teammate", 1000);

    pubUAV = node_->create_publisher<visualization_msgs::msg::Marker>(topic_name_prefix + "uav_visualization", 100);
    pubMeshUAV = node_->create_publisher<visualization_msgs::msg::Marker>(topic_name_prefix + "uav_mesh_visualization", 100);
    pubCluster = node_->create_publisher<visualization_msgs::msg::Marker>(topic_name_prefix + "cluster_visualization", 100);
    pubPredictRegionInput = node_->create_publisher<sensor_msgs::msg::PointCloud2>(topic_name_prefix + "cluster_input", 100);
    pubHighIntenInput = node_->create_publisher<sensor_msgs::msg::PointCloud2>(topic_name_prefix + "high_intensity_input", 100);
    pubPredictRegion = node_->create_publisher<visualization_msgs::msg::Marker>(topic_name_prefix + "predict_region", 100);
    pubTempTracker = node_->create_publisher<visualization_msgs::msg::Marker>(topic_name_prefix + "temp_tracker", 100);
    pubTeammateList = node_->create_publisher<swarm_msgs::msg::ConnectedTeammateList>(topic_name_prefix + "connected_teammate_list", 10);
    pubTeammateNum = node_->create_publisher<std_msgs::msg::Int8>(topic_name_prefix + "connected_teammate_num", 10);
    pubTeammateIdTrajMatching = node_->create_publisher<swarm_msgs::msg::ConnectedTeammateList>("/teammate_id_with_traj_matching", 10);

    pubTeammateOdom_.resize(MAX_UAV_NUM);
    pubTeammateTraj_.resize(MAX_UAV_NUM);
    for (int i = 0; i < MAX_UAV_NUM; ++i) {
        pubTeammateOdom_[i] = node_->create_publisher<nav_msgs::msg::Odometry>(topic_name_prefix + "teammate_odom/UAV" + SetString(i), 100);
        pubTeammateTraj_[i] = node_->create_publisher<visualization_msgs::msg::MarkerArray>(topic_name_prefix + "teammate_traj/UAV" + SetString(i), 100);
    }
    temp_tracker.clear();
...
void Multi_UAV::PublishQuadstate(const V3D &unbiased_gyr, const double &lidar_end_time, const double &first_lidar_time) {
    ...
    quadstate_msg_pub.header.stamp = rclcpp::Time(static_cast<int64_t>(lidar_end_time * 1e9));
...
void Multi_UAV::PublishGlobalExtrinsic(const double &lidar_end_time) {
    ...
    global_extrinsic_msg.header.stamp = rclcpp::Time(static_cast<int64_t>(lidar_end_time * 1e9));
...
bool Multi_UAV::LoadParam(string param_name, vector<T> &param_value, vector<T> default_value) {
    if (!node_->has_parameter(param_name)) {
        node_->declare_parameter<vector<T>>(param_name, default_value);
    }
    if (node_->get_parameter(param_name, param_value)) {
        printf("\033[0;32m Load param %s success: \033[0;0m", param_name.c_str());
...
#pragma omp parallel for
    ...
    *cluster_cloud = all_predict_region_cloud;
    ...
    if (pubPredictRegionInput->get_subscription_count() > 0) {
        ...
        cluster_input_msg.header.stamp = rclcpp::Time(static_cast<int64_t>(lidar_end_time * 1e9));
...
void Multi_UAV::VisualizeText(rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr pub, const double &time, const int &id, const double &scale, const V3D &pos, const string &text, const V3D &color){
   //Publish Text
   visualization_msgs::msg::Marker vis_text;
   vis_text.header.stamp = rclcpp::Time(static_cast<int64_t>(time * 1e9));
...
void Multi_UAV::VisualizeBoundingBox(rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr pub, const double &time, const int &id, const V3D &color, const V3D &pos, const double &size){
   //Publish Bounding box
   visualization_msgs::msg::Marker line_strip;
   line_strip.header.stamp = rclcpp::Time(static_cast<int64_t>(time * 1e9));
...
void Multi_UAV::VisualizeDeleteAllCluster(const double &time) {
   ...
   vis_cluster_delete.header.stamp = rclcpp::Time(static_cast<int64_t>(time * 1e9));
...
void Multi_UAV::PublishTeammateOdom(const double &lidar_end_time){
   ...
   for (auto iter = teammate_tracker.begin(); iter != teammate_tracker.end(); ++iter){
       ...
       cout << teammate_id << "  ";
       if(pubTeammateOdom_[teammate_id]->get_subscription_count() < 1)
           continue;
       ...
       TeammateOdom.header.stamp = rclcpp::Time(static_cast<int64_t>(lidar_end_time * 1e9));
       ...
       pubTeammateOdom_[teammate_id]->publish(TeammateOdom);
   }
   cout << RESET << endl;
}
...
void Multi_UAV::PublishConnectedTeammateList(const double &lidar_end_time){
   ...
   list.header.stamp = rclcpp::Time(static_cast<int64_t>(lidar_end_time * 1e9));
...
void Multi_UAV::VisualizeTeammateTracker(const double &lidar_end_time, const int &teammate_id) {
   ...
   if (iter != teammate_tracker.end()) {
       ...
       vis_uav_delete.header.stamp = rclcpp::Time(static_cast<int64_t>(lidar_end_time * 1e9));
...
void Multi_UAV::VisualizeTeammateTrajectory(rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr pub_, deque<Vector4d> &traj,
                                           const V3D position,
   ...
   for (auto cur_index: traj) {
       ...
       line_list.header.stamp = rclcpp::Time(static_cast<int64_t>(timestamp * 1e9));
...
void Multi_UAV::VisualizeTeammateTrajectorySphere(rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr pub_, deque<Vector4d> &traj,
                                           const V3D position,
                                           const string namespace_,
                                           const double mkr_size,
                                           const double &timestamp,
   ...
   switch (teammate_id) {
       case 1: teammate_traj_color2 = V3D(1.0, 0.25, 0.25); break; //red
       case 2: teammate_traj_color2 = V3D(0, 1.0, 0.5); break; //green
       //case 3: teammate_traj_color2 = V3D(1, 0.65, 0.0); break; //orange (removed duplicate)
       case 3: teammate_traj_color2 = V3D(0, 0.75, 1); break; //blue
       case 4: teammate_traj_color2 = V3D(1, 1, 0); break; //yellow
       case 5: teammate_traj_color2 = V3D(0.42, 0.35, 1.0); break; //purple
       default: teammate_traj_color2 = V3D(0.5, 0.5, 0.5); break; //gray
...
void Multi_UAV::VisualizeMeshUAV(const double &lidar_end_time, const int &teammate_id, const Teammate &teammate){
   ...
   if (iter != teammate_tracker.end()) {
       ...
       VisualizeTeammateTrajectory(pubTeammateTraj_[teammate_id], teammate_traj[teammate_id],
...
void Multi_UAV::VisualizeTemporaryPredictRegion(const double &lidar_end_time) {
   ...
   sphere.header.stamp = rclcpp::Time(static_cast<int64_t>(lidar_end_time * 1e9));
...
void Multi_UAV::VisualizeRectangle(rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr pub_rect, const double &lidar_end_time, const int &rect_id, const V3D &position, const V3D rect_size) {
   visualization_msgs::msg::Marker line_strip;
