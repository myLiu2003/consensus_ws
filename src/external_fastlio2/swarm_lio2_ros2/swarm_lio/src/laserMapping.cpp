...
bool runtime_pos_log = false, pcd_save_en = false, path_en = true, mavros_pose_en = false;
...
void VisualizeDrone(rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr &pub) {
    Quaterniond quat;
    ...
    vis_drone.header.stamp = rclcpp::Time(static_cast<int64_t>(lidar_end_time * 1e9));
    ...
    meshROS.header.stamp = rclcpp::Time(static_cast<int64_t>(lidar_end_time * 1e9));
    meshROS.ns = "mesh";
...
void imu_prop_callback() {
    ...
    if (imu_prop_enable && !prop_imu_buffer.empty()) {
        ...
        ts1.transform.rotation = imu_prop_odom.pose.pose.orientation;
        static auto br1 = std::make_shared<tf2_ros::TransformBroadcaster>(g_node);
...
void SigHandle(int sig) {
    ...
    flg_exit = true;
    RCLCPP_WARN(rclcpp::get_logger("swarm_lio"), "catch sig %d", sig);
...
void livox_pcl_cbk(const livox_ros_driver2::msg::CustomMsg::SharedPtr &msg) {
    ...
    if (rclcpp::Time(msg->header.stamp).seconds() < last_timestamp_lidar) {
        RCLCPP_WARN(rclcpp::get_logger("swarm_lio"), "lidar loop back, clear buffer");
        lidar_buffer.clear();
        time_buffer.clear();
    }
    ...
    if(accumulate_frame_en && !cut_frame_en){
        ...
        static double first_lidar_head_time = rclcpp::Time(msg->header.stamp).seconds();
        //合帧2s，合成后的每一帧的点云数目和10Hz点云相同
        if(scan_count < 2 * original_frequency){
            if(scan_count % (original_frequency/10) == 0)
                first_lidar_head_time = rclcpp::Time(msg->header.stamp).seconds();
...
void standard_pcl_cbk(const sensor_msgs::msg::PointCloud2::SharedPtr &msg) {
    ...
    mtx_buffer_lidar.lock();
    if (rclcpp::Time(msg->header.stamp).seconds() < last_timestamp_lidar) {
        RCLCPP_ERROR(rclcpp::get_logger("swarm_lio"), "lidar loop back, clear Lidar buffer.");
        lidar_buffer.clear();
        time_buffer.clear();
    }
    last_timestamp_lidar = rclcpp::Time(msg->header.stamp).seconds();
...
void imu_cbk(const sensor_msgs::msg::Imu::SharedPtr &msg_in) {
    ...
    uint64_t _ns = (uint64_t)msg_in->header.stamp.sec * 1000000000ULL + msg_in->header.stamp.nanosec - (int64_t)(time_offset_lidar_imu * 1e9); msg->header.stamp.sec = _ns / 1000000000ULL; msg->header.stamp.nanosec = _ns % 1000000000ULL;
    ...
    if (timestamp < last_timestamp_imu) {
        RCLCPP_WARN(rclcpp::get_logger("swarm_lio"), "IMU loop back, clear IMU buffer.");
        imu_buffer.clear();
    }
    ...
    if (!lidar_pushed) {
        ...
        *(meas.lidar) = lidar_sorted;
        if (meas.lidar->points.size() <= 1) {
            RCLCPP_WARN(rclcpp::get_logger("swarm_lio"), "Too few input point cloud!\n");
            mtx_buffer_lidar.lock();
...
void publish_frame_world(rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pubCloudRegistered, rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pubCloudRegisteredSparse) {
    if (scan_pub_en) {
        ...
        //Publish
        laserCloudmsg.header.stamp = laserCloudSparsemsg.header.stamp = rclcpp::Time(static_cast<int64_t>(lidar_end_time * 1e9));
...
void publish_frame_body(rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pubCloudRegisteredBody_) {
...
void publish_odometry(rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr pubLidarSlamOdom) {
    ...
    ts2.header.stamp = odomAftMapped.header.stamp;
    ts2.header.frame_id = topic_name_prefix + "world";
    ts2.child_frame_id = "quad" + SetString(drone_id) + "_aft_mapped";
    ts2.transform.translation.x = odomAftMapped.pose.pose.position.x;
    ts2.transform.translation.y = odomAftMapped.pose.pose.position.y;
    ts2.transform.translation.z = odomAftMapped.pose.pose.position.z;
    ts2.transform.rotation = odomAftMapped.pose.pose.orientation;
    static auto br = std::make_shared<tf2_ros::TransformBroadcaster>(g_node);
    br->sendTransform(ts2);
}

//=== publish_mavros removed (ROS2 port) ===
...
void VisualizeRectangle(rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr pub_rect, const V3D &head, const V3D rect_size, const int &rect_id) {
    visualization_msgs::msg::Marker line_strip;
    line_strip.header.stamp = rclcpp::Time(static_cast<int64_t>(lidar_end_time * 1e9));
...
void point_downsample_filt_rectangle(PointCloudXYZI &orig_pcl, PointCloudXYZI &pcl_out,
                                     const shared_ptr<Multi_UAV> &swarm_in, rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr pub_rect) {
    ...
    vis_cluster_delete.header.stamp = rclcpp::Time(static_cast<int64_t>(lidar_end_time * 1e9));
...
void gt_pos_cbk_sim(const nav_msgs::msg::Odometry::SharedPtr &msg) {
    VD(7) gt;
...
void gt_pos_cbk_real(const geometry_msgs::msg::PoseStamped::ConstPtr &msg) {
...
int main(int argc, char** argv) {
    ...
    auto node = std::make_shared<rclcpp::Node>("swarm_lio");
    g_node = node.get();
    node->declare_parameter("common/lid_topic", "/livox/lidar");  node->get_parameter("common/lid_topic", lid_topic);
    node->declare_parameter("common/imu_topic", "/livox/imu");  node->get_parameter("common/imu_topic", imu_topic);
    node->declare_parameter("common/time_offset_lidar_imu", 0.0);  node->get_parameter("common/time_offset_lidar_imu", time_offset_lidar_imu);
    node->declare_parameter("common/drone_id", 0);  node->get_parameter("common/drone_id", drone_id);
    node->declare_parameter("filter_size_corner", 0.5);  node->get_parameter("filter_size_corner", filter_size_corner_min);
    node->declare_parameter("mapping/filter_size_surf", 0.5);  node->get_parameter("mapping/filter_size_surf", filter_size_surf_min);
    node->declare_parameter("mapping/filter_size_map", 0.5);  node->get_parameter("mapping/filter_size_map", filter_size_map_min);
    node->declare_parameter("mapping/cube_side_length", 200.0);  node->get_parameter("mapping/cube_side_length", cube_len);
    node->declare_parameter("mapping/det_range", 300.f);  node->get_parameter("mapping/det_range", DET_RANGE);
    node->declare_parameter("mapping/imu_en", true);  node->get_parameter("mapping/imu_en", imu_en);
    node->declare_parameter("mapping/gyr_cov", 0.1);  node->get_parameter("mapping/gyr_cov", gyr_cov);
    node->declare_parameter("mapping/acc_cov", 0.1);  node->get_parameter("mapping/acc_cov", acc_cov);
    node->declare_parameter("mapping/grav_cov", 0.001);  node->get_parameter("mapping/grav_cov", grav_cov);
    node->declare_parameter("mapping/b_gyr_cov", 0.0001);  node->get_parameter("mapping/b_gyr_cov", b_gyr_cov);
    node->declare_parameter("mapping/b_acc_cov", 0.0001);  node->get_parameter("mapping/b_acc_cov", b_acc_cov);
    node->declare_parameter("mapping/point_filter_num", 2);  node->get_parameter("mapping/point_filter_num", point_filter_num);
    node->declare_parameter("mapping/max_iteration", 4);  node->get_parameter("mapping/max_iteration", NUM_MAX_ITERATIONS);
    node->declare_parameter("preprocess/blind", 1.0);  node->get_parameter("preprocess/blind", p_pre->blind);
    node->declare_parameter("preprocess/lidar_type", 1); int _lt; node->get_parameter("preprocess/lidar_type", _lt); lidar_type = (LID_TYPE)_lt;
    node->declare_parameter("preprocess/scan_line", 16);  node->get_parameter("preprocess/scan_line", p_pre->N_SCANS);
    node->declare_parameter("preprocess/feature_extract_en", false);  node->get_parameter("preprocess/feature_extract_en", p_pre->feature_enabled);
    node->declare_parameter("calibration/cut_frame_en", false);  node->get_parameter("calibration/cut_frame_en", cut_frame_en);
    node->declare_parameter("calibration/cut_frame_num", 1);  node->get_parameter("calibration/cut_frame_num", cut_frame_num);
    node->declare_parameter("calibration/accumulate_frame_en", false);  node->get_parameter("calibration/accumulate_frame_en", accumulate_frame_en);
    node->declare_parameter("calibration/original_frequency", 10);  node->get_parameter("calibration/original_frequency", original_frequency);
    node->declare_parameter("calibration/gravity_align_enable", true);  node->get_parameter("calibration/gravity_align_enable", gravity_align_en);
    node->declare_parameter("calibration/gravity_align_time", 50.0);  node->get_parameter("calibration/gravity_align_time", gravity_align_dur);
    node->declare_parameter("publish/path_en", true);  node->get_parameter("publish/path_en", path_en);
    node->declare_parameter("publish/scan_publish_en", true);  node->get_parameter("publish/scan_publish_en", scan_pub_en);
    node->declare_parameter("publish/dense_publish_en", true);  node->get_parameter("publish/dense_publish_en", dense_pub_en);
    node->declare_parameter("publish/scan_bodyframe_pub_en", false);  node->get_parameter("publish/scan_bodyframe_pub_en", scan_body_pub_en);
    node->declare_parameter("publish/mavros_pose_en", false);  node->get_parameter("publish/mavros_pose_en", mavros_pose_en);
    node->declare_parameter("runtime_pos_log_enable", false);  node->get_parameter("runtime_pos_log_enable", runtime_pos_log);
    node->declare_parameter("log_dir", "/tmp/swarm_lio");  node->get_parameter("log_dir", root_dir);
    node->declare_parameter("pcd_save/pcd_save_en", false);  node->get_parameter("pcd_save/pcd_save_en", pcd_save_en);
    node->declare_parameter("pcd_save/pcd_resolution", 0.03);  node->get_parameter("pcd_save/pcd_resolution", pcd_resolution);
    node->declare_parameter("pcd_save/interval", -1);  node->get_parameter("pcd_save/interval", pcd_save_interval);
    node->declare_parameter("mapping/LI_extrinsic_T", vector<double>()); node->get_parameter("mapping/LI_extrinsic_T", extrinT);
    node->declare_parameter("mapping/LI_extrinsic_R", vector<double>()); node->get_parameter("mapping/LI_extrinsic_R", extrinR);
    node->declare_parameter("sub_gt_pose_topic", "");  node->get_parameter("sub_gt_pose_topic", sub_gt_pose_topic);

    node->declare_parameter("multiuav/actual_uav_num", 5);  node->get_parameter("multiuav/actual_uav_num", actual_uav_num);
    node->declare_parameter("multiuav/frame_num_in_sliding_window", original_frequency/10);  node->get_parameter("multiuav/frame_num_in_sliding_window", frame_num_in_sliding_window);
    node->declare_parameter("multiuav/mutual_observe_noise", 0.001);  node->get_parameter("multiuav/mutual_observe_noise", mutual_observe_noise_);
    node->declare_parameter("multiuav/degeneration_thresh", 30);  node->get_parameter("multiuav/degeneration_thresh", degeneration_thresh);
    node->declare_parameter("imu_propagate/enable", false);  node->get_parameter("imu_propagate/enable", imu_prop_enable);
    node->declare_parameter("imu_propagate/topic", "lidar_slam/imu_propagate");  node->get_parameter("imu_propagate/topic", imu_prop_topic);
    node->declare_parameter("imu_propagate/filter_acc_en", false);  node->get_parameter("imu_propagate/filter_acc_en", filter_acc_en);
...
//    }
    ...
    //Swarm LIO
    shared_ptr<Multi_UAV> swarm(new Multi_UAV(g_node, drone_id));

    //Gravity Alignment
    G_T_PX4.setIdentity();
    G_T_I0.setIdentity();
    vector<double> PX4_R_I_deg, PX4_p_I, level_horizon_deg;
    node->declare_parameter("calibration/PX4_R_I", vector<double>()); node->get_parameter("calibration/PX4_R_I", PX4_R_I_deg);
    node->declare_parameter("calibration/PX4_p_I", vector<double>()); node->get_parameter("calibration/PX4_p_I", PX4_p_I);
    ...
    node->declare_parameter("calibration/level_horizon", vector<double>()); node->get_parameter("calibration/level_horizon", level_horizon_deg);
    ...
    fout_out.open(root_dir + "/quad" + SetString(drone_id) + "_state.txt", ios::out);
    fout_pose.open(root_dir + "/quad" + SetString(drone_id) + "_pose.txt", ios::out);
//    fout_global_extrin.open(root_dir + "/quad" + SetString(drone_id) + "_global_extrinsic.txt", ios::out);
    fout_solve_time.open(root_dir + "/quad" + SetString(drone_id) + "_solve_time.txt", ios::out);
    fout_cpu_memory.open(root_dir + "/quad" + SetString(drone_id) + "_cpu_memory.txt", ios::out);
//    fout_cov.open(root_dir + "/quad" + SetString(drone_id) + "_cov.txt", ios::out);
    if (fout_out)
    ...
    if (lidar_type == SIM) {
        sub_pcl2 = node->create_subscription<sensor_msgs::msg::PointCloud2>(
            lid_topic, rclcpp::SensorDataQoS(),
            [](const sensor_msgs::msg::PointCloud2::SharedPtr m) { standard_pcl_cbk(m); });
        RCLCPP_INFO(node->get_logger(), "Simulation lidar input: %s (PointCloud2)", lid_topic.c_str());
    } else {
        sub_livox = node->create_subscription<livox_ros_driver2::msg::CustomMsg>(
            lid_topic, rclcpp::SensorDataQoS(),
            [](const livox_ros_driver2::msg::CustomMsg::SharedPtr m) { livox_pcl_cbk(m); });
        RCLCPP_INFO(node->get_logger(), "Livox lidar input: %s (CustomMsg)", lid_topic.c_str());
    }

    auto sub_imu = node->create_subscription<sensor_msgs::msg::Imu>(
        imu_topic, rclcpp::SensorDataQoS(),
        [](const sensor_msgs::msg::Imu::SharedPtr m){ imu_cbk(m); });

    auto pubCloudRegistered = node->create_publisher<sensor_msgs::msg::PointCloud2>
            (topic_name_prefix + "cloud_registered", 1000);        // Undistorted raw pointcloud in world frame
    auto pubCloudRegisteredSparse = node->create_publisher<sensor_msgs::msg::PointCloud2>
            (topic_name_prefix + "cloud_registered_sparse", 1000);        // Downsampled world-frame cloud
    auto pubCloudRegisteredBody = node->create_publisher<sensor_msgs::msg::PointCloud2>
            (topic_name_prefix + "cloud_registered_body", 1000);   // Undistorted pointcloud in body frame
    pub_map = node->create_publisher<sensor_msgs::msg::PointCloud2>
            (topic_name_prefix + "downsampled_map", 1000);
    ...
    traj_pub = node->create_publisher<visualization_msgs::msg::MarkerArray>(topic_name_prefix + "traj", 100);

    pubImuPropOdom = node->create_publisher<nav_msgs::msg::Odometry> (topic_name_prefix + imu_prop_topic, 1000);
    ...
    pubFilteredImu = node->create_publisher<sensor_msgs::msg::Imu>(topic_name_prefix + "imu/data_filtered", 1000);


    // groundtruth_subscriber — only SIM mode, real mode removed (type conflict)
    text_scale = 0.3;
    mesh_scale = 0.8;
    // groundtruth not used in real flight
    ground_truth.clear();
    ...
    if (mavros_pose_en) {
        mavros_pose_publisher = node->create_publisher<geometry_msgs::msg::PoseStamped>("/mavros/vision_pose/pose", 10);
    }
    ...
    rclcpp::Time main_loop_start_time = rclcpp::Clock().now();
...
//                cout << "---------------------------------------" <<  RESET <<  endl;
            ...
            if (flg_reset) {
                RCLCPP_WARN(rclcpp::get_logger("swarm_lio"), "reset when rosbag play back.");
                p_imu->Reset();
                flg_reset = false;
                continue;
            }
            ...
            if(gravity_align_en){
                if (p_imu->imu_need_init_) {
                    main_loop_start_time = rclcpp::Clock().now();
                }
                double wait_duration = (rclcpp::Clock().now() - main_loop_start_time).seconds();
            ...
            if (feats_undistort_orig_lidar->empty() || (feats_undistort_orig_lidar == NULL)) {
                RCLCPP_WARN(rclcpp::get_logger("swarm_lio"), "FAST-LIO not ready, no points stored.");
                continue;
            }
...
//                cout << svd.singularValues().transpose() << endl;
                ...
                if (!EKF_stop_flg && (rematch_num >= 2 || (iterCount == NUM_MAX_ITERATIONS - 1))) {
                    if (flg_EKF_inited) {
                        ...
                        geoQuat = createQuaternionFromRPY
