...
struct orgtype
{
  double range, dista, intersect;
  double angle[2];
  E_jump edj[2];
  Feature ftype;
  orgtype() { range = 0; edj[Prev] = Nr_nor; edj[Next] = Nr_nor; ftype = Nor; intersect = 2; }
};
...
class Preprocess
{
  ...
  void process_accumulate_frame_livox(const livox_ros_driver2::msg::CustomMsg::SharedPtr &msg, PointCloudXYZI::Ptr &pcl_out, const double &first_lidar_head_time);
