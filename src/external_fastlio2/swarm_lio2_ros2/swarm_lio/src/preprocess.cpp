...
void Preprocess::process_accumulate_frame_livox(const livox_ros_driver2::msg::CustomMsg::SharedPtr &msg,
    ...
    for (uint i = 1; i < plsize; i++) {
            ...
            ((msg->points[i].tag & 0x30) == 0x00)) //保留第0个回波
        {
            ...
            //use curvature as time of each laser points，unit: ms
            pl_full[i].curvature =
                    msg->points[i].offset_time / float(1e6) + rclcpp::Time(msg->header.stamp).seconds() - first_lidar_head_time;
